...
cloudflareStream.media.model.Query = wp.media.model.Query.extend( {
	...
	sync: function( method, model, options ) {
		...
		if ( 'read' === method ) {
			...
			let timestampOffset = '';
			if ( cloudflareStream.media.model.Attachments.all.models.length > 0 ) {
				timestampOffset = '&end=' + cloudflareStream.media.model.Attachments.all.models[ cloudflareStream.media.model.Attachments.all.models.length - 1 ].attributes.modified.toISOString();
			}
	...
	get: ( function() {
		...
		return function( props, options ) {
			...
			_.each( props, function( value, prop ) {
				...
				args[ cloudflareStream.media.model.Query.propmap[ prop ] || prop ] = value;
			} );

			// Fill any other default query args.
			_.defaults( args, cloudflareStream.media.model.Query.defaultArgs );
			...
			if ( ! query ) {
				query = new cloudflareStream.media.model.Query( [], _.extend( options || {}, {
					props: props,
					args: args,
				} ) );
				queries.push( query );
